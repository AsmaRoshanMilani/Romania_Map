...
class SearchAgent_UCS { // ------------------------------Intelligent
	...
	public Node solve(RomaniaProblem prblm) {
		...
		startNode.pathCost = startNode.pathCost + heuristic(startNode.toString());
		...
		do {
			...
			if (prblm.goalTest(currentNode.state)) {
				System.out.println("Number of pushed nodes into fringe : " + fringeCount + " ");
				System.out.println("Number of expanded nodes : " + exploredcount + " " + "\n" + explored);
				return currentNode;
			}
			exploredcount++;
			currentNode.pathCost = currentNode.pathCost - heuristic(currentNode.toString());
			Action[] children = new Action[] {};
			children = prblm.succFunc(currentNode.state);
			for (int i = 0; i < children.length; i++) {
				Node child = new Node(children[i].successor);
				child.pathCost = currentNode.pathCost + children[i].cost + heuristic(child.state.toString());
				child.parent = currentNode;
				boolean infringe = false;
				Node[] fringeList = fringe.toArray(new Node[fringe.size()]);
				for (int j = 0; j < fringeList.length; j++) {
					if ((fringeList[j].state.city).compareTo(child.toString()) == 0) {
						infringe = true;
						break;
					}
				}
				if (infringe == false && !explored.contains(child.state.toString())) {
					fringe.add(child);
					fringeCount++;
				}
